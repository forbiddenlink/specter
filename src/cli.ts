...
// Import first-run experience
import { runFirstTimeExperience } from './first-run.js'
...
program
  ...
  .action(async () => {
    ...
    const didFirstRun = await runFirstTimeExperience(rootDir)
    if (!didFirstRun) {
      // Not first run — show banner + help (existing behavior)
      printBanner()
      program.outputHelp()
    }
  })
