...
function generateReviewBody(analyses: FileAnalysis[], personality: PersonalityMode): string {
  ...
  if (personality === 'roast') {
    ...
    const opening = applyPersonality('In this city of code, every PR tells a story...', personality)
    body += `${opening}\n\n`
  } else if (personality === 'dramatic') {
    const opening = applyPersonality('Behold! A pull request emerges from the depths!', personality)
    body += `${opening}\n\n`
  }
