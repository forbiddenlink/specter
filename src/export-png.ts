...
function calculateHeight(
  ...
  return Math.max(400, Math.ceil(padding * 2 + headerHeight + textHeight + watermarkHeight));
}
...
function detectContentType(content: string): 'wrapped' | 'achievements' | 'dna' | 'generic' {
  ...
  if (content.includes('DNA PROFILE') || content.includes('DOUBLE HELIX') || content.includes('Sequence:')) {
    return 'dna';
  }
  return 'generic';
}
...
export async function exportToPng(
  ...
  // Calculate height
  const calculatedHeight = options.height || calculateHeight(
    parsedLines,
    tempCtx,
    fontSize,
    lineHeight,
  ...
  if (watermark) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('\u{1F47B} Generated by Specter  |  Share your code story', width / 2, watermarkY);
  }
...
function drawDecorations(
  ...
  ctx.strokeStyle = theme === 'dark' ? `rgba(255, 255, 255, ${alpha * 0.3})` : `rgba(0, 0, 0, ${alpha * 0.3})`;
  ctx.lineWidth = 1;
  ...
  const circleColor = contentType === 'wrapped' ? '#6b5ce7' :
                      contentType === 'achievements' ? '#f9ca24' :
                      contentType === 'dna' ? '#00d2d3' : '#6b5ce7';

  // Top-right decorative circle
  ctx.beginPath();
  ctx.arc(width - 80, 80, 120, 0, Math.PI * 2);
  ctx.fillStyle = `${circleColor}${Math.floor(alpha * 255).toString(16).padStart(2, '0')}`;
  ctx.fill();

  // Bottom-left decorative circle
  ctx.beginPath();
  ctx.arc(80, height - 80, 100, 0, Math.PI * 2);
  ctx.fillStyle = `${circleColor}${Math.floor(alpha * 0.5 * 255).toString(16).padStart(2, '0')}`;
  ctx.fill();
}
