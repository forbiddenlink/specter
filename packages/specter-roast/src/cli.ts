...
program
  ...
  .action(async (directory: string, options: { json?: boolean; mild?: boolean; savage?: boolean }) => {
    ...
    if (options.json) {
      console.log(JSON.stringify({ command: 'roast', stats, timestamp: new Date().toISOString() }, null, 2));
      return;
    }
...
function generateRoast(stats: CodebaseStats, options: RoastOptions = {}): string {
  ...
  lines.push('');
  lines.push(chalk.italic(savage
  ...
  if (savage) {
    lines.push(chalk.white(`  ${stats.fileCount} files. That's ${stats.fileCount} opportunities for bugs.`));
    lines.push(chalk.white(`  ${stats.totalLines.toLocaleString()} lines of code. That's ${stats.totalLines.toLocaleString()} lines of technical debt.`));
  } else if (mild) {
    lines.push(chalk.white(`  ${stats.fileCount} files with ${stats.totalLines.toLocaleString()} lines of code.`));
  ...
  if (stats.largestFiles.length > 0) {
    const biggest = stats.largestFiles[0];
  ...
  if (stats.todoCount > 0) {
    ...
    if (savage) {
      lines.push(chalk.white(`  ${stats.todoCount} TODOs and FIXMEs. That's ${stats.todoCount} promises you made to yourself and broke.`));
      ...
      lines.push(chalk.white(`  ${stats.todoCount} TODOs and FIXMEs. They're not reminders, they're monuments to procrastination.`));
  ...
  if (stats.consoleLogCount > 0) {
    ...
    if (savage) {
      lines.push(chalk.white(`  ${stats.consoleLogCount} console.logs. Production is going to love those.`));
      ...
      lines.push(chalk.white(`  ${stats.consoleLogCount} console.logs. Were these for debugging? Because they're debugging your reputation.`));
  ...
  if (stats.anyCount > 0) {
    ...
    if (savage) {
      lines.push(chalk.white(`  ${stats.anyCount} uses of 'any'. Why even use TypeScript at this point?`));
      ...
      lines.push(chalk.white(`  ${stats.anyCount} uses of 'any'. That's ${stats.anyCount} times you gave up on type safety.`));
  ...
  if (stats.deepestNesting > 8) {
    ...
    if (savage) {
      lines.push(chalk.white(`  Maximum nesting depth: ${stats.deepestNesting}. That's not code, that's an archaeological dig.`));
      ...
      lines.push(chalk.white(`  Maximum nesting depth: ${stats.deepestNesting}. That's Inception-level complexity.`));
  ...
  if (stats.longFunctions > 0) {
    ...
    if (savage) {
      lines.push(chalk.white(`  ${stats.longFunctions} functions over 50 lines. These aren't functions, they're entire applications.`));
      ...
      lines.push(chalk.white(`  ${stats.longFunctions} functions over 50 lines. That's not a function, that's job security.`));
  ...
  if (stats.duplicateLikelyFiles.length > 0) {
    ...
    lines.push(chalk.white(`  Found ${Math.floor(stats.duplicateLikelyFiles.length / 2)} pairs of suspiciously similar files:`));
    for (let i = 0; i < stats.duplicateLikelyFiles.length; i += 2) {
      lines.push(chalk.dim(`    ${stats.duplicateLikelyFiles[i]} \u{2194}\u{FE0F} ${stats.duplicateLikelyFiles[i + 1]}`));
    }
  ...
  if (stats.nodeModulesSize > 500) {
    ...
    if (savage) {
      ...
      lines.push(chalk.white(`  ${stats.nodeModulesSize} packages. That's modern JavaScript for you.`));
    } else {
      lines.push(chalk.white(`  ${stats.nodeModulesSize} packages in node_modules.`));
